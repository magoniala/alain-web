...
export async function POST(req: Request) {
  ...
  const { subject_eu, body_eu, subject_es, body_es, programado_para } = body;
  ...
  const { data, error } = await supabase
    .from("newsletter_campanas")
    .insert({ subject_eu, body_eu, subject_es, body_es, programado_para })
    .select()
    .single();
...
export async function PATCH(req: Request) {
  if (!auth(req)) return NextResponse.json({ error: "No autorizado." }, { status: 401 });
  const { id, subject_eu, body_eu, subject_es, body_es, programado_para } = await req.json();
  if (!id) return NextResponse.json({ error: "Falta id." }, { status: 400 });
  const { error } = await supabase
    .from("newsletter_campanas")
    .update({ subject_eu, body_eu, subject_es, body_es, programado_para })
    .eq("id", id)
