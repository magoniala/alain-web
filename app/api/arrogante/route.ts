...
export async function POST(req: Request) {
  ...
  const base = current ?? {
    personas_testadas: 0,
    creen_que_diran_su_nombre: 0,
    suma_gilipollas: 0,
    media_gilipollas: null,
  };

  const creeDirian =
    cree_que_diran_su_nombre === "si" ||
    cree_que_diran_su_nombre === "probablemente-si";
  ...
  const sumaActual =
    base.suma_gilipollas && base.suma_gilipollas > 0
      ? base.suma_gilipollas
      : base.media_gilipollas && personasActuales > 0
        ? base.media_gilipollas * personasActuales
        : 0;
  ...
  const statsUpdate: Record<string, number> = {
    ...
    creen_que_diran_su_nombre:
      (base.creen_que_diran_su_nombre ?? 0) + (creeDirian ? 1 : 0),
  ...
  if (email?.trim() && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.trim())) {
    await supabase
      ...
      .upsert(
        { email: email.trim(), origen: "arrogante" },
        { onConflict: "email", ignoreDuplicates: true },
      );
    const mailBD = await cargarMailSecuencia("arrogante", 1, "es", {
      tiktok: TIKTOK_URL,
    });
    const cuerpoArrogante =
      mailBD?.cuerpo ??
      `
    ...
    // El pie (contacto y enlace de baja) se añade aquí, no se guarda en la
    // tabla: el enlace de baja es obligatorio y no debe poder borrarse desde
    // el panel por accidente.
    await sendEmail(
      email.trim(),
      mailBD?.asunto || "Tu respuesta ya está dentro del experimento.",
      `
        <div style="font-family:Georgia,serif;max-width:560px;margin:0 auto;padding:2.5rem 2rem;color:#1a1a1a;background:#ffffff;">
          <div style="font-size:1.05rem;line-height:2;color:#1a1a1a;">${cuerpoArrogante}</div>
