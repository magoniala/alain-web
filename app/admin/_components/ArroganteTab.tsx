...
export default function ArroganteTab() {
  ...
  async function loadAll() {
    const [s, f, e, n] = await Promise.all([
      ...
      fetch("/api/arrogante/emails", { headers: authHeaders() }).then(r => r.json()),
      fetch("/api/arrogante/nominados", { headers: authHeaders() }).then(r => r.json()),
    ]);
    setStats(s);
    setEditStats(s);
    setFrases(f);
  ...
  async function moveNominado(id: string, direction: "up" | "down") {
    ...
    await Promise.all([
      fetch("/api/arrogante/nominados", { method: "PATCH", headers: authHeaders(), body: JSON.stringify({ id: normalized[idx].id, posicion: normalized[idx].posicion }) }),
      fetch("/api/arrogante/nominados", { method: "PATCH", headers: authHeaders(), body: JSON.stringify({ id: normalized[swapIdx].id, posicion: normalized[swapIdx].posicion }) }),
    ]);
  }
  ...
  async function addNominado() {
    ...
    setNewNominadoTexto("");
    const n = await fetch("/api/arrogante/nominados", { headers: authHeaders() }).then(r => r.json());
    setNominados(n);
