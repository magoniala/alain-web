...
const MOMENTOS_OPTIONS = [
  "Aperitiboa (bazkaria edo afaria baino lehenago)",
  "Bazkaria",
  "Bazkari ondorengo ospakizuna",
  "Afaria",
  "Afari ondorengo ospakizuna",
  "Koktela / Zutikako momentua",
  "Oraindik zehaztu gabe",
];
...
export default function BelaustegiPage() {
  ...
  const goNext = async () => {
    ...
    if (screen === 2 && !formData.fechaAproximada.trim()) {
      setError("Mesedez, adierazi data gutxi gorabehera.");
      return;
    }
    ...
    if (screen === 6) {
      ...
      try {
        const payload = {
          ...formData,
          tipoEvento: formData.tipoEvento === "Beste bat" && formData.tipoEventoOtro
            ? `Beste bat — ${formData.tipoEventoOtro}`
            : formData.tipoEvento,
        };
        ...
        return (
          <div key={2}>
            ...
            <div style={fieldStyle}>
              <input
                type="text"
                placeholder="2025eko ekaina, udazken honetan, oraindik ez dakigu..."
      ...
      case 5:
        return (
          <div key={5}>
            ...
            <p style={{ fontSize: "0.82rem", color: "rgba(242,242,240,0.40)", letterSpacing: "0.08em", marginBottom: "1.6rem" }}>
              Aukerakoa — kontuan hartzeko beste edozein xehetasun
            </p>
            <div style={{ borderBottom: "1px solid rgba(242,242,240,0.12)" }}>
              <textarea
                placeholder="Idatzi hemen zure erantzuna..."
        ...
        return (
          <div key={7}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "1.4rem" }}
              ...
              <p>Formularioa jasota.</p>
              <p>Ahalik eta azkarren bidaliko dizut ekitaldira egokitutako proposamena.</p>
            </div>
          </div>
        );
      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[#0B0B0C]/70 backdrop-blur-md">
        <div className="mx-auto flex max-w-[1400px] items-center justify-between px-8 py-4 md:px-16">
          ...
          <div style={{ display: "flex", alignItems: "center", border: "1px solid rgba(242,242,240,0.16)", fontSize: "0.75rem", letterSpacing: "0.1em" }}>
            ...
            <Link href="/es/belaustegi" style={{ padding: "0.3rem 0.65rem", color: "rgba(242,242,240,0.55)", textDecoration: "none", transition: "color 0.2s" }} onMouseEnter={e => (e.currentTarget.style.color = "rgba(242,242,240,0.90)")} onMouseLeave={e => (e.currentTarget.style.color = "rgba(242,242,240,0.55)")}>ES</Link>
          </div>
        </div>
      </header>
      ...
      <section className="mx-auto flex min-h-[38vh] max-w-[1400px] flex-col justify-end px-8 pb-16 md:px-16 md:pb-20">
        <div>
          ...
          <h1 className="max-w-[820px] text-[clamp(2.4rem,4.5vw,4.2rem)] font-medium leading-[1.06] tracking-[-0.03em]">
            Ekitaldi berezietarako formularioa
          </h1>
        </div>
      </section>
