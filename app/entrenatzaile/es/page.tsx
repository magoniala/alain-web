...
export default function ValoracionEntrenatzaileEsPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!formData.turno) {
      setError("Indica cuándo sueles tener más tiempo libre.");
      return;
    }
    ...
    try {
      const res = await fetch("/api/entrenatzaile/valoracion", {
        ...
        body: JSON.stringify({ ...formData, edad: edadNum, idioma: "es" }),
      });
      ...
      <section className="fade-in mx-auto max-w-[1400px] px-8 pb-24 md:px-16">
        <div className="max-w-[680px]">
          ...
          <div style={{ marginTop: "2.5rem" }}>
            <MapaCTA label="Quiero mi mapa gratis" />
          </div>
        </div>
      </section>
